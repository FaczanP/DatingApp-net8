...
public class PhotoRepository(DataContext context) : IPhotoRepository
{
    ...
    public async Task<IEnumerable<PhotoForApprovalDto>> GetUnapprovaldPhotos()
    {
        return await context.Photos.IgnoreQueryFilters().Where(p => p.IsApproved == false).Select(u =>
